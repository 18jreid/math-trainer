...
import { CheckCircle2, XCircle, RotateCcw, Stethoscope, HeartPulse, Timer, ClipboardList, Trophy } from "lucide-react";
...
function recordLabel(field: NewRecord["field"]): string {
  if (field === "both")  return "New record — best score & fastest time!";
  if (field === "score") return "New record — best score!";
  if (field === "time")  return "New record — fastest avg time!";
  return "";
}

export default function ResultsScreen({ results, onRetry, onHome, newRecord }: ResultsScreenProps) {
  const correct  = results.filter((r) => r.correct).length;
  const total    = results.length;
  const pct      = Math.round((correct / total) * 100);
  const avgTime  = Math.round(results.reduce((s, r) => s + r.timeMs, 0) / total);
  const grade    = getGrade(pct);
  const streak   = results.reduce((best, _, i) => {
    let run = 0;
    ...
    <div className="min-h-screen grid-bg flex flex-col items-center justify-center px-4 py-12">
      ...
      >
        ...
        <div className="flex items-center justify-center gap-2 mb-6">
          ...
          <span className="text-[#00c9a7] text-xs font-mono uppercase tracking-widest">Session Report</span>
        </div>
        ...
        {newRecord?.isNewRecord && (
          <motion.div
            ...
            className="border border-[#fbbf2440] bg-[#fbbf2408] rounded-2xl px-4 py-3 flex items-center gap-3 mb-5"
          >
            <Trophy className="w-5 h-5 text-[#fbbf24] shrink-0" />
            <p className="text-[#fbbf24] text-sm font-semibold">{recordLabel(newRecord.field)}</p>
          </motion.div>
        )}
        ...
        <div className="text-center mb-7">
          ...
          >
            <span className={`text-3xl font-bold font-mono ${grade.color} ${grade.glow}`}>{pct}%</span>
        ...
        {/* Case Review */}
        <div className="border border-[#163040] rounded-2xl bg-[#091418] overflow-hidden mb-5">
          <div className="px-5 py-3 border-b border-[#163040] flex items-center gap-2">
            <div className="w-1.5 h-1.5 rounded-full bg-[#00c9a7]" />
            <h3 className="text-[#6b9ea8] text-xs font-mono uppercase tracking-widest">Case Review</h3>
          </div>
          <div className="divide-y divide-[#0e1f24] max-h-56 overflow-y-auto">
            {results.map((r, i) => (
              <motion.div
                key={i}
              ...
              >
                <div className="mt-0.5 shrink-0">
                  {r.correct ? <CheckCircle2 className="w-4 h-4 text-[#22c55e]" /> : <XCircle className="w-4 h-4 text-[#ef4444]" />}
                </div>
        ...
        <div className="flex gap-3">
          ...
          >
            <Stethoscope className="w-4 h-4" /> New Session
          </button>
          <motion.button
